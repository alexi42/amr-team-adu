...
class GridCell():
    ...
    def convert_world_coordinates_to_grid(self, coord):
        ...
        row = int(round((x - START[0]) / RESOLUTION))
        col = int(round((y - START[1]) / RESOLUTION))
        return row, col
...
class FollowWaypoints(smach.State):
    ...
    def check_if_waypoint_in_obstacle(self):
        """
        Check if the next waypoint is too close to a detected obstacle.
        Both waypoint and obstacle are compared in base_link frame.
        """
        ...
        waypoint_world = self.path_waypoints[0]
        waypoint_base = self.transform_to_base_link(waypoint_world)
        ...
        for obstacle in obstacles:
            distance = np.linalg.norm(waypoint_base - obstacle)
