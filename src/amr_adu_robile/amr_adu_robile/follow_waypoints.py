...
class FollowWaypoints(smach.State):
    ...
    """
    def __init__(self, node, k_a=0.2, k_r=0.2, rho_0=0.5,
                 max_linear_velocity=0.2, max_angular_velocity=0.1):
    ...
    def publish_local_plan(self, obstacles, horizon=3.0, dt=0.1):
        ...
        with self.lock:
            ...
            obstacles_world = [
                transform_to_world(self.robot_position, self.robot_angle, obstacle)
    ...
    def control_loop(self, q_goal, obstacles, robot_position, robot_angle, k_a, k_r, linear_gain, max_linear_velocity, max_angular_velocity):
        ...
        q_goal_base = transform_to_base_link(robot_position, robot_angle, q_goal)

        attractive_force = self.calculate_attractive_force(k_a, q_base, q_goal_base)
        repulsive_force = self.calculate_repulsive_force(k_r, q_base, obstacles)
        ...
        if force_magnitude > 1e-6:
            force_direction = total_force / force_magnitude
            linear_vel = np.clip(force_magnitude * linear_gain,
    ...
    def calculate_attractive_force(self, k_a, q, q_goal):
        """
        ...
        return k_a * direction / distance

    def calculate_repulsive_force(self, k_r, q, obstacles):
        """
        ...
        for obstacle_pos in obstacles:
            ...
            repulsive_force += k_r * term1 * term2 * direction
    ...
    def execute(self, userdata):
        with self.rlock:
            ...
            dest_grid = convert_world_coordinates_to_grid(
                np.array(self.node.DESTINATION), self.node.START, self.node.RESOLUTION
            )

            start = self.node.START
            res = self.node.RESOLUTION
            robot_position = self.robot_position
            robot_angle = self.robot_angle
            obstacles = self.obstacles
            path_waypoints = self.path_waypoints
            if obstacles is not None and path_waypoints is not None:
                for wp in path_waypoints:
                    for obstacle in obstacles:
                        wp_grid = convert_world_coordinates_to_grid(wp, start, res)
                        obstacle_world = transform_to_world(robot_position, robot_angle, obstacle)
                        obstacle_grid = convert_world_coordinates_to_grid(obstacle_world, start, res)
            ...
            if np.linalg.norm(next_waypoint - current_robot_position) < epsilon:
                ...
                if len(self.path_waypoints) == 0:
                    ...
                    robot_grid = convert_world_coordinates_to_grid(
                        current_robot_position, self.node.START, self.node.RESOLUTION
                    )
                    ...
                    threshold = max(self.node.RESOLUTION * 1.5, 0.1)
                    if dist < threshold:
                        twist.linear.x = 0.0
                        twist.linear.y = 0.0
                        self.cmd_vel_pub.publish(twist)
            ...
            twist = self.control_loop(self.q_goal, self.obstacles,
                                      self.robot_position, self.robot_angle,
                                      self.k_a, self.k_r, self.linear_gain,
